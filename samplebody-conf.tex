...
The position of each particle in the population is affected by the influence of global leaders and local leaders, while also being affected by a randomly chosen particle from the population to induce some stochasticity in the optimization pipeline. We first model the influence of the global leader on the local leaders and the influences of the local leaders  on each population element using equation \eqref{one} and \eqref{two}. We introduce a hierarchical crossover between the two influencing equations governed by a hierarchical crossover parameter.
...
The updated position of the particle $x$ is governed by the hierarchical crossover operation and a mutation operation. The hierarchical operation is affected by the global leader $g_L$ and the local leader $l$ through the parametric equations \eqref{one} and \eqref{two}. Switching between the two is governed by the hierarchical crossover parameter $P_HC$.

The new position of particle $x$ is governed by the position of the global leader $g_L$ and the corresponding local leader $l$ of the particle. We introduce two parametric equations governing the generation of new position vector. The first equation \eqref{one}, regulates the co-operative influences of the global leader $g_L$ and the local leader $l$, where their influences are governed by leadership intensity parameters $\lambda_1$ and $\lambda_2$. Equation \eqref{two} deals with the influence of local leader on the particle $x$ governed by the influential parameters $\lambda_3$ (controlling influence of leader on particle) and $k_1$ (controlling degree of co-operation of particle). The given equations are as follows:

\begin{equation}
\label{one}
E_g = \lambda_1 g_L + \lambda_2 l
\end{equation}

\begin{equation}
\label{two}
E_g = \lambda_3 l + k_1 x
\end{equation}

Finally, a new particle position is determined using the unified effect of \eqref{one} and \eqref{two} as shown:
...
Where, $c$ is a randomly chosen particle from the population to induce stochasticity in the system,
and $k$ is a normalized parameter controlling the extent of stochastic influence.

\begin{algorithm}
\caption{Distributed Leader Optimization Algorithm}
\label{algo}
\begin{algorithmic}[1]
	\Procedure{Start}{}
		...
		\While{termination criteria is not met}
			\For{each individual $x_i$ in $P$}
				...
				\State Generate $E_g$ and $E_l$ as shown in \eqref{one} and \eqref{two}.
				\State Compute new position vector $y$ using \eqref{three}.
				\If {$f(y) \textless f(x)$}
